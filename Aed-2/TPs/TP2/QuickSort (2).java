...
public class QuickSort {
    ...
    int reparticao(int passagem[], int embaixo, int Emcima)
    {
        int pivot = passagem[Emcima]; 
        int i = (embaixo-1); 
        for (int j=embaixo; j<Emcima; j++)
        {
            if (passagem[j] <= pivot)
            {
                i++;
                ...
                int temp = passagem[i];
                passagem[i] = passagem[j];
                passagem[j] = temp;
            }
        }
  
        int temp = passagem[i+1];
        passagem[i+1] = passagem[Emcima];
        passagem[Emcima] = temp;
  
        return i+1;
    }
  

    void QuickSortFunction(int passagem[], int embaixo, int Emcima)
    {
        if (embaixo < Emcima)
        {
            
            int pi = reparticao(passagem, embaixo, Emcima);
  
            QuickSortFunction(passagem, embaixo, pi-1);
            QuickSortFunction(passagem, pi+1, Emcima);
        }
    }
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			while ((arquivo = conteudoCsv.readLine()) != null) {
				for (y = 0; y < x; y++) {
					if (arquivo.contains(pubin[y])) {
						...
						novaOrdem[y] = musica.getDuration_ms();
			...
			int p = 0;
			int o = 0;
QuickSort pp = new QuickSort();
int ll = novaOrdem.length;
pp.QuickSortFunction(novaOrdem,0,ll-1);
			...
			for (p = 0; p < 10; p = p + 1) {
				for (o = 0; o < y; o = o + 1) {
					if (dedo[o].contains(String.valueOf( novaOrdem[p]))) {
