...
public class orderselect {
	...
	public static String[] ordenador(String[] pit, int f) { //funcao OrderSelect 
        for (int i = 0; i < f -1; i = i + 1) {

            int dex = i;
            String dexString = pit[i];
            for (int j = i + 1; j < f ; j = j + 1) {

                if (pit[j].compareTo(dexString) < 0) { //compara um caractere com o proximo da lista 

                    dexString = pit[j];//se o proximo for menor que o atual a troca e efetuada
                    dex = j;
                }
            }
            if (dex != i) {// reindexador
                String temp = pit[dex];
                pit[dex] = pit[i];
                pit[i] = temp;
            }
	...
	public static Musica colocaNaClasse(String pit) { // seta tudo
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			String[] novaOrdem = new String[10000];
		...
		String[] aA = ordenador(novaOrdem, y);// recebe o array ja organizado
				...
				for (o = 0; o < y; o = o + 1) {
					if (dedo[o].contains(aA[p])) {
						System.out.println(dedo[o]); 
					     dedo[o] = ""; //se conter o elemento do aa printa o do dedo
					}
