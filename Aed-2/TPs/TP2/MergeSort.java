...
class Musica {//declaracao das variaveis
...
//metodos get/set
	...
	public String toString() {//print da classe
...
public class MergeSort {
	...
	void SoftMerge(Musica[] arr, int l, int m, int r) {

		int Valor1 = m - l + 1;
		int Valor2 = r - m;

		Musica L[] = new Musica[Valor1];

		Musica R[] = new Musica[Valor2];

		for (int i = 0; i < Valor1; ++i) // passagem de valor para os auxiliares
			L[i] = arr[l + i];
		for (int j = 0; j < Valor2; ++j) // dois for para que possa ser realizada a divisao em duas linhas de comparacao
			R[j] = arr[m + 1 + j];
		...
		while (i < Valor1 && j < Valor2) {
			if (L[i].getEnergy() <= R[j].getEnergy()) {// so utilizo o energy em comparacoes pois posso puxar a classe
														// como um todo
				arr[k] = L[i];
				i++;
			} else {
				arr[k] = R[j]; // todas as igualdades sao realizadas com a classe como um todo pois ela nao
								// depende do energy
				j++;
			}
			k++;
	...
	void SortOfMerge(Musica[] arr, int l, int r) {
		if (l < r) {

			int m = l + (r - l) / 2;

			SortOfMerge(arr, l, m);
			SortOfMerge(arr, m + 1, r);

			SoftMerge(arr, l, m, r);
		}
	}

	public static Musica[] ordenador(Musica[] pit, int f) { // reutilização do orderselect Comentarios adicionais no
															// programa do orderSelect
...
//main
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			double[] novaOrdem = new double[100];
			...
			int p = 0;
			int o = 0;
			MergeSort pp = new MergeSort();
			int ll = ooo.length;
			ordenador(ooo, y);
			pp.SortOfMerge(ooo, 0, ll - 1);
...
//funcao para gerar e escrever no arquivo de texto
			File aqr = new File("689661_mergesort.txt");
			aqr.createNewFile();
