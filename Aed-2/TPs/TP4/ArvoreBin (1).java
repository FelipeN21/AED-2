...
class ArvoreBinaria {
	...
	//metodo pesquisar com parametragem simples
	public String pesquisar(String x) {
		return pesquisar(x, raiz);
	}

	//pesquisa e mostra o caminho
	...
	//acessa a raiz e devolve seu valor
...
//leitura dos dados na main
    public static void main(String[] args) throws Exception {
        ...
        int cot = 0;
        pubin2[cot] = MyIO.readString();
        while (Vef(pubin2[cot]) != false) {
            cot++;
            pubin2[cot] = MyIO.readString();
            comps++;
        }
        ...
        try {
            ...
            ArvoreBinaria haha = new ArvoreBinaria();
           // Lista lista = new Lista(6);
            ...
            while ((arquivo = conteudoCsv.readLine()) != null) {
                ...
                for (int y2 = 0;y2 < cot;y2++) {

                    if (arquivo.contains(pubin2[y2])) {
                        musica2 = colocaNaClasse(arquivo);
                       // System.out.println(pubin2[y2]);
            ...
            for (int t = 0; t < x; t++) {
                for (int j = 0; j < ky; j++) {
                    if (musicarray2[j].id.contains(pubin[t])) {
                        haha.inserir(musicarray2[j].id);              
                        comps++;
                    }
                }
            }
		//pesquisando e mostrando o caminho
            for (int tb = 0; tb < ky2; tb++) {
                System.out.println(pubin2[tb]);
                System.out.print("raiz ");
              System.out.print(haha.pesquisar(pubin2[tb])+ "\n");
            }
           


            File aqr = new File( "689661_ArvoreBinaria.txt" );
           aqr.createNewFile();
           FileWriter fw = new FileWriter( aqr );
           long fimt = System.currentTimeMillis();
           fw.write("689661"+ "\t" + (fimt - inity) +"\t" + comps);
           fw.close();
