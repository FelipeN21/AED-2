...
class Lista {
    ...
    public void inserirFim(Musica musica) throws Exception {
        if (n >= array.length) {
    ...
    public String remover(int posicao) throws Exception {
        ...
        String resp = array[posicao].name;
        n--;
           ...
           for(int t = 0;t < x;t++ )
           {
               for(int j = 0;j < ky;j++)
               {
                   if(musicarray2[j].id.contains(pubin[t]))
                   {
                      ...
                      lista.inserirFim(musicarray2[j]);
                   }
               }
           }
           ...
           while(lo < holder)
           {
                ...
                for(int navega = 0;navega < ky2 ;navega = navega + 1)
                   {
                    if(idadd.equals(musicarray[navega].getId()))
                    {
                        ...
                        lista.inserir(musicarray[navega], posicao);
                        res = res + 1;
               ...
               if(junc[lo].contains("IF")){
                ...
                for(int navega = 0;navega < ky2 ;navega = navega + 1)
                {
                    ...
                    if(idadd.equals(musicarray[navega].getId()))
                    {
                        lista.inserirFim(musicarray[navega]);
                        res = res + 1;
               ...
               { 
                String[] aux = junc[lo].split(" ");
                int posicao = Integer.parseInt(aux[1]);
                       String stt =  lista.remover(posicao);
                       System.out.println("(R) "+ stt);
                       res = res + 1;
               }
              if(junc[lo].contains("RF"))
               {
                  String prf = lista.removerFim();
